...
void OLED_Num(u8 x,u8 y,u16 num,u8 TextSize)
{  
	char* str="0";
	switch(num)
	{
...
void Shownum(u8 x,u8 y,u16 num ,u8 size)
{
	u8 ge,shi,bai,qian,wan;
	short int NNum=(short int)num;
	if(NNum<0)
	{
		 OLED_ShowStr(x,y,"-", size);
		NNum=-NNum;
	}
	else
	{
		...
		x+=(size+size+4);
		OLED_Num(x,y,wan,size);
		x+=(size+size+4);
		OLED_Num(x,y,qian,size);
		x+=(size+size+4);
		OLED_Num(x,y,bai,size);
		x+=(size+size+4);
		OLED_Num(x,y,shi,size);
		x+=(size+size+4);
		OLED_Num(x,y,ge,size);			
}
